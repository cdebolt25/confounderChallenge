...
ggplot(patients_df, aes(x = patient, y = post_surgical_score, 
  ...
  annotate("text", x = max(patients_df$patient) * 1.01, 
           y = mean_dreamy, label = paste("○", round(mean_dreamy, 2)), 
           color = "#4E79A7", size = 4, hjust = 0, fontface = "bold") +
  annotate("text", x = max(patients_df$patient) * 1.01, 
           y = mean_duck, label = paste("△", round(mean_duck, 2)), 
           color = "#E15759", size = 4, hjust = 0, fontface = "bold") +
...
ggplot(patients_randomized_df, aes(x = patient, y = post_surgical_score, 
  ...
  # Text annotations for means with shape symbols
  annotate("text", x = max(patients_randomized_df$patient) * 1.01, 
...
ggplot(patients_df, aes(x = severity, y = post_surgical_score, 
  ...
  annotate("text", x = max(patients_df$severity) * 1.02, 
