...
ggplot(patients_df, aes(x = patient, y = post_surgical_score, 
  ...
  geom_hline(yintercept = mean_dreamy, linetype = "dashed", 
             color = "#4E79A7", linewidth = 0.7) +
  geom_hline(yintercept = mean_duck, linetype = "dashed", 
             color = "#E15759", linewidth = 0.7) +
  # Text annotations for means with shape symbols
  annotate("text", x = max(patients_df$patient) * 0.97, 
           y = mean_dreamy, label = paste("○", round(mean_dreamy, 2)), 
           color = "#4E79A7", size = 4, hjust = 1, fontface = "bold") +
  annotate("text", x = max(patients_df$patient) * 0.97, 
           y = mean_duck, label = paste("△", round(mean_duck, 2)), 
           color = "#E15759", size = 4, hjust = 1, fontface = "bold") +
...
As readers of @fig-plot-outcomes-r, the easiest path for our brains is to accept the mental model of surgical outcomes depicted in @fig-example-node.

![DAG Model Explaining Surgical Outcomes](dag-example-node.png){#fig-example-node width=50%}

The obvious conclusion from using the mental model of @fig-example-node and the data shown in @fig-plot-outcomes-r is that **Doc Dreamy is the superior surgeon because his patients' scores are lower than Doc Duck's patients' scores**.
...
# Create the plot (same styling as Figure 1)
ggplot(patients_randomized_df, aes(x = patient, y = post_surgical_score, 
                                    color = doctor_name, shape = doctor_name)) +
  geom_point(size = 2.5, alpha = 0.7) +
  # Horizontal dashed lines for means
  geom_hline(yintercept = mean_dreamy_rand, linetype = "dashed", 
             color = "#4E79A7", linewidth = 0.8) +
  geom_hline(yintercept = mean_duck_rand, linetype = "dashed", 
             color = "#E15759", linewidth = 0.8) +
  # Text annotations for means with shape symbols
  annotate("text", x = max(patients_randomized_df$patient) * 0.97, 
           y = mean_dreamy_rand, label = paste("○", round(mean_dreamy_rand, 2)), 
           color = "#4E79A7", size = 4, hjust = 1, fontface = "bold") +
  annotate("text", x = max(patients_randomized_df$patient) * 0.97, 
           y = mean_duck_rand, label = paste("△", round(mean_duck_rand, 2)), 
           color = "#E15759", size = 4, hjust = 1, fontface = "bold") +
  # Colorblind-friendly colors and shapes
  scale_color_manual(
    name = "Doctor",
  ...
  theme_minimal(base_size = 12) +
  theme(
    plot.title = element_text(size = 12.8, face = "bold", hjust = 0.5, margin = margin(b = 15)),
    axis.title = element_text(size = 11, face = "bold"),
    axis.text = element_text(size = 10),
    legend.title = element_text(size = 11, face = "bold"),
    legend.text = element_text(size = 10),
...
mean_dreamy_severity <- mean(patients_df$post_surgical_score[patients_df$doctor_name == "Doc Dreamy"])
mean_duck_severity <- mean(patients_df$post_surgical_score[patients_df$doctor_name == "Doc Duck"])
...
ggplot(patients_df, aes(x = severity, y = post_surgical_score, 
  ...
  geom_point(size = 2.5, alpha = 0.7) +
  # Horizontal dashed lines for means
  geom_hline(yintercept = mean_dreamy_severity, linetype = "dashed", 
             color = "#4E79A7", linewidth = 0.8) +
  geom_hline(yintercept = mean_duck_severity, linetype = "dashed", 
             color = "#E15759", linewidth = 0.8) +
  # Text annotations for means positioned appropriately
  annotate("text", x = max(patients_df$severity) * 0.98, 
           y = mean_dreamy_severity, label = paste("○", round(mean_dreamy_severity, 2)), 
           color = "#4E79A7", size = 4, hjust = 1, fontface = "bold") +
  annotate("text", x = max(patients_df$severity) * 0.98, 
           y = mean_duck_severity, label = paste("△", round(mean_duck_severity, 2)), 
           color = "#E15759", size = 4, hjust = 1, fontface = "bold") +
  # Colorblind-friendly colors and shapes (same scheme as previous figures)
  scale_color_manual(
    name = "Doctor",
    values = c("Doc Dreamy" = "#4E79A7", "Doc Duck" = "#E15759"),
    labels = c("Doc Dreamy", "Doc Duck")
...
Looking at @fig-plot-outcomes-severity-r, the value of stratifying by severity is visually most obvious with initial patient severity scores between -1 and 1 (highlighted in the overlay). We chose this region because it shows substantial overlap between the two surgeons—both surgeons have considerable data within this range, making it an ideal region for comparing their performance. Outside of this range, there is less data available for making direct comparisons: Doc Duck sees more of the higher-severity patients (severity > 1), while Doc Dreamy sees more of the lower-severity patients (severity < -1).
...
## Conclusion: Lessons from the Confounder
